...
/**
  \brief   sets the root node of the subgraph to be rendered, nullptr for models root node
  \param   ...
  \remarks ...
*/
...
/*virtual*/
//void
//PurrRenderer::setRenderGraphRoot(std::shared_ptr<model::Node> startNode)
//{
...
#if 0
  std::cerr << std::endl;
  std::cerr << std::endl << "PurrRenderer::resize(): ";
  std::cerr << std::endl << "   Creating new GBuffer";
  std::cerr << std::endl << "   width:  " << screenWidth;
  std::cerr << std::endl << "   height: " << screenHeight;
  std::cerr << std::endl;
#endif // 0
