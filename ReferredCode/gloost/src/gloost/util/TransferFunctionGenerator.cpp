...
{
  auto doc = gloost::openXmlFile(filePath);
  if(!doc)
  {
...
TransferFunctionGenerator::createFromXmlElement(tinyxml2::XMLElement* localRoot)
{
  ...
  int fd_version = 0;
  gloost::interpretFromChar(fd_version, localRoot->Attribute("fd_version"));
  if (fd_version != 0)
  {
    return shared_ptr(new TransferFunctionGenerator( interpolation, segmentation, resolution, label));
  }
  ...
  {
    everythingIsFine &= gloost::interpretFromChar(label, localRoot->Attribute("label"));
    if(label.empty())
    {
      label = "noLabel";
    }
  ...
  if (localRoot->Attribute("interpolation"))
  {
    ...
    everythingIsFine &= gloost::interpretFromChar(stateString, localRoot->Attribute("interpolation"));
  ...
  if (localRoot->Attribute("segmentation"))
  {
    ...
    everythingIsFine &= gloost::interpretFromChar(stateString, localRoot->Attribute("segmentation"));
  ...
  {
    everythingIsFine &= gloost::interpretFromChar(resolution, localRoot->Attribute("resolution"));
  }
  ...
  if (lowerBoundNode)
  {
    everythingIsFine &= gloost::interpretFromChar(instance->_lowerBoundColor.r, lowerBoundNode->Attribute("r"));
    everythingIsFine &= gloost::interpretFromChar(instance->_lowerBoundColor.g, lowerBoundNode->Attribute("g"));
    everythingIsFine &= gloost::interpretFromChar(instance->_lowerBoundColor.b, lowerBoundNode->Attribute("b"));
    everythingIsFine &= gloost::interpretFromChar(instance->_lowerBoundColor.a, lowerBoundNode->Attribute("a"));
  }
  // parse lower bound
  tinyxml2::XMLElement* upperBoundNode = localRoot->FirstChildElement("upper_bound");
  if (upperBoundNode)
  {
    everythingIsFine &= gloost::interpretFromChar(instance->_upperBoundColor.r, upperBoundNode->Attribute("r"));
    everythingIsFine &= gloost::interpretFromChar(instance->_upperBoundColor.g, upperBoundNode->Attribute("g"));
    everythingIsFine &= gloost::interpretFromChar(instance->_upperBoundColor.b, upperBoundNode->Attribute("b"));
    everythingIsFine &= gloost::interpretFromChar(instance->_upperBoundColor.a, upperBoundNode->Attribute("a"));
  }
  ...
  while (nodeElement)
  {
    ...
    {
      everythingIsFine &= gloost::interpretFromChar(pos, nodeElement->Attribute("pos"));
    }
    if(nodeElement->Attribute("r"))
    {
      everythingIsFine &= gloost::interpretFromChar(color_r, nodeElement->Attribute("r"));
    }
    if(nodeElement->Attribute("g"))
    {
      everythingIsFine &= gloost::interpretFromChar(color_g, nodeElement->Attribute("g"));
    }
    if(nodeElement->Attribute("b"))
    {
      everythingIsFine &= gloost::interpretFromChar(color_b, nodeElement->Attribute("b"));
    }
    if(nodeElement->Attribute("a"))
    {
      everythingIsFine &= gloost::interpretFromChar(color_a, nodeElement->Attribute("a"));
    }
    if(nodeElement->Attribute("showvalue"))
    {
      everythingIsFine &= gloost::interpretFromChar(showValue, nodeElement->Attribute("showvalue"));
    }
    if(nodeElement->Attribute("label"))
    {
      everythingIsFine &= gloost::interpretFromChar(label, nodeElement->Attribute("label"));
    }
  ...
  while (annotationTag)
  {
    ...
    {
      everythingIsFine &= gloost::interpretFromChar(annotationPos, annotationTag->Attribute("pos"));
    }
    if(annotationTag->Attribute("label"))
    {
      everythingIsFine &= gloost::interpretFromChar(label, annotationTag->Attribute("label"));
    }
    if(annotationTag->Attribute("showvalue"))
    {
      everythingIsFine &= gloost::interpretFromChar(showValue, annotationTag->Attribute("showvalue"));
    }
...
TransferFunctionGenerator::writeToXml(const gloost::PathType& path)
{
  ...
  if(gloost::writeXmlFile(path, newDoc))
  {
    return true;
  }
  return false;
...
TransferFunctionGenerator::toXmlElement(const std::shared_ptr<tinyxml2::XMLDocument>& xmlDocument)
{
  ...
  lowerBoundNode->SetAttribute("r", gloost::toString(_lowerBoundColor.r).c_str());
  lowerBoundNode->SetAttribute("g", gloost::toString(_lowerBoundColor.g).c_str());
  lowerBoundNode->SetAttribute("b", gloost::toString(_lowerBoundColor.b).c_str());
  lowerBoundNode->SetAttribute("a", gloost::toString(_lowerBoundColor.a).c_str());

  auto upperBoundNode =  xmlDocument->NewElement("upper_bound");
  element->InsertEndChild(upperBoundNode);
  upperBoundNode->SetAttribute("r", gloost::toString(_upperBoundColor.r).c_str());
  upperBoundNode->SetAttribute("g", gloost::toString(_upperBoundColor.g).c_str());
  upperBoundNode->SetAttribute("b", gloost::toString(_upperBoundColor.b).c_str());
  upperBoundNode->SetAttribute("a", gloost::toString(_upperBoundColor.a).c_str());
  ...
  for (const auto& colorNode : getNodeVector())
  {
    ...
    node->SetAttribute("pos", gloost::floatToString(colorNode._position).c_str());
    node->SetAttribute("r",   gloost::floatToString(colorNode._color.r).c_str());
    node->SetAttribute("g",   gloost::floatToString(colorNode._color.g).c_str());
    node->SetAttribute("b",   gloost::floatToString(colorNode._color.b).c_str());
    node->SetAttribute("a",   gloost::floatToString(colorNode._color.a).c_str());
  ...
  for (const auto& annotation : getAnnotationVector())
  {
    ...
    annotationElement->SetAttribute("pos", gloost::floatToString(annotation._position).c_str());
...
{
  if (_interpolation == Interpolation::DISCRETE || _interpolation == Interpolation::DISCRETE_RANGE)
  {
...
/**
  \brief   generates a texture with discrete color ranges by using the color of the end node of a range
  \param   ...
  \remarks ...
*/
...
/**
  \brief   generates a gradient texture by interpolating linear betwen nodes
  \param   ...
  \remarks ...
*/
...
/**
  \brief   returns a vector of nodes
  \param   ...
  \remarks ...
*/
