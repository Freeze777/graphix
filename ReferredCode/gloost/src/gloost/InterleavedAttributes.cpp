...
//*/
//
//InterleavedAttributes::LayoutVector&
//InterleavedAttributes::getLayout()
//{
//  return _layout;
//}
...
InterleavedAttributes::getLayoutString() const
{
  ...
  for (unsigned int i=0; i!=_layout.size(); ++i)
  {
    ...
    layoutString += "layout(location=" + gloost::toString(i) + ") in " + typeString + " " +  _layout[i]._name + ";\n";
  }
...
InterleavedAttributes::writeToFile(gloost::BinaryFile& outFile) const
{
  ...
  outFile.writeString(gloost::toString(_layout.size()) + " ");


  for (unsigned l=0; l!=_layout.size(); ++l)
  {
    outFile.writeString(_layout[l]._name + " ");
    outFile.writeString(gloost::toString(_layout[l]._numElements) + " ");
    outFile.writeString(gloost::toString(_layout[l]._stride) + " ");
  }

  outFile.writeString(gloost::toString(_vector.size()) + " ");
