...
TextureText::TextureText(const std::string& fontTextureFileName):
    _fontTextureId(0),
    _fontTexture(0),
    _writePosX(0),
    _writePosY(400),
    ...
    _charSpace(0),
    _lineSpace(0),
    _lineAlignment(GLOOST_TEXTURETEXT_ALIGN_LEFT),
    _charAlpha(1.0),
    ...
    _texManager(TextureManager::getInstance())
{
  ...
  _fontTexture->setTexParameter(GL_TEXTURE_MIN_FILTER, GL_NEAREST);
  _fontTexture->setTexParameter(GL_TEXTURE_MAG_FILTER, GL_NEAREST);
...
TextureText::renderTextLine(float offsetX, const std::string& textLine)
{
  ...
  if (_lineAlignment == (unsigned)GLOOST_TEXTURETEXT_ALIGN_CENTER)
  {
    alignmentOffsetX = (int)((-lineLength-0.5)*(_charWidth+_charSpace)*_charScale/2.0f);
  }
  else if (_lineAlignment == (unsigned)GLOOST_TEXTURETEXT_ALIGN_RIGHT)
  {
  ...
  glPushMatrix();
  {
    ...
    for (unsigned i=0; i != lineLength; ++i )
    {
      ...
      glPushMatrix();
      {
        glTranslatef(i*(_charWidth+_charSpace)*_charScale, _writePosY + _charHeight*_charScale, 0.0);
//        glRotated(180, 0.0, 0.0, 1.0);
        glScalef(_charWidth*_charScale, -_charHeight*_charScale, 1.0);
        gloost::gl::drawQuad(texCoords.r, texCoords.g,
...
void
TextureText::renderFreeLine(unsigned howMuche)
{
...
void
TextureText::setWritePosition(float posX, float posY)
{
  _writePosX = posX;
  _writePosY = posY;
}
...
void
TextureText::setLineAlignment(unsigned lineAlignment)
{
