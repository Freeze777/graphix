...
Message::asFormatedString() const
{
  ...
  const auto levelStringLength = gloost::toString(_logLevel).length();
  const auto fillString        = gloost::stringRepeat(" ", levelStringLength+6u);
	const auto outString         = gloost::stringSearchAndReplace(asString(), "\n", "\n" + fillString);
