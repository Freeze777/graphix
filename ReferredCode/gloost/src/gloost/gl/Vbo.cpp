...
#ifdef DEBUG_COUNT_INSTANCES
  ++_instanceCounter;
  std::cerr << std::endl << "vbo, adds to:  " << _instanceCounter;
#endif // DEBUG_COUNT_INSTANCES
}
...
Vbo::~Vbo()
{
//	std::cerr << std::endl << "Vbo::~Vbo(): " << this;
...
#ifdef DEBUG_COUNT_INSTANCES
  --_instanceCounter;
  std::cerr << std::endl << "vbo, taken to: " << _instanceCounter;
#endif // DEBUG_COUNT_INSTANCES
...
//  CheckErrorsGL("Before gloost::obsolete::Vbo::initInContext()");
...
//  {
//    packageStride += attribInfo[i]._stride;  // TODO
//  }
...
//  CheckErrorsGL("After gloost::obsolete::Vbo::initInContext()");
...
Vbo::removeFromContext(unsigned contextId)
{
  ...
  {
//    std::cerr << std::endl << "remove vbo on: " << getResourceHandleForContext(contextId, GLOOST_VBO_RESOURCE_VBO);
...
//    std::cerr << std::endl << "testing vao: " << getResourceHandleForContext(contextId, GLOOST_VBO_RESOURCE_VAO);
  if (handleInContextExists(contextId, GLOOST_VBO_RESOURCE_VAO))
  {
//    std::cerr << std::endl << "remove vao on: " << getResourceHandleForContext(contextId, GLOOST_VBO_RESOURCE_VAO);
