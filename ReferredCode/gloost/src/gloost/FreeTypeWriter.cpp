...
{
  _currentWritePosition[1] -= _lineHeight + _lineSpace;
}
...
FreeTypeWriter::write(const std::string& text)
{
  ...
  if (_glyphSpecs.size() < 33)
  {
    return;
  }

  Point3 pos = _currentWritePosition;
  ...
  for (unsigned int c=0; c!=text.length(); ++c)
  {
    ...
    const auto& specs = _glyphSpecs[index];

    glPushMatrix();
    {
      glTranslatef( (int) pos[0],
                    (int) pos[1] - (specs._height-specs._topOffset), 0.0);

      if (_forcedGlyphWidth)
      {
        pos[0] += _forcedGlyphWidth;
      }
      else
      {
        pos[0] += (specs._width + _charSpace);
      }

      glScalef( specs._width,
                specs._height,
                1.0f);
      /// draw some stuff
      gl::drawQuad(specs._texcoords);
    }
    glPopMatrix();
    ++index;
